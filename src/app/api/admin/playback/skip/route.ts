...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await runProviderControl(userId, 'skip', {
      userId,
      deviceId: device_id,
    });
    ...
    if (!result.ok) {
      ...
      return NextResponse.json(
        { error: result.message || 'Failed to skip track', code: result.code },
        { status }
      );
    }
    ...
    console.error('Error skipping playback:', error);
    return NextResponse.json(
      {
