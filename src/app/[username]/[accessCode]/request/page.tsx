...
export default function UserRequestPage() {
  ...
  const accessCode = decodeURIComponent(params.accessCode as string);
  ...
  const verifyAccess = async (codeValue?: string, token?: string) => {
    setVerifying(true);
    setAccessError('');
    setIsLoading(true);
    ...
    try {
      ...
      if (response.ok) {
        ...
        sessionStorage.setItem(
          `event_auth_${username}`,
          JSON.stringify({
            eventId: data.event.id,
            accessCode: data.event.accessCode || codeValue,
  ...
  // Auto-verify from URL access code (or legacy bypass token)
  useEffect(() => {
    if (bypassToken) {
      verifyAccess(undefined, bypassToken);
      return;
    }
    if (accessCode && isValidAccessCodeFormat(accessCode)) {
      verifyAccess(accessCode);
      return;
    }
    setAccessError('Invalid access code in URL');
    setIsLoading(false);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [username, accessCode, bypassToken]);
  ...
  const searchTracks = async (query: string) => {
    ...
    try {
      const response = await axios.get<SearchResult>(`${API_BASE}/spotify/search`, {
        params: { q: query.trim(), username, accessCode, limit: 20 },
        timeout: 15000,
      ...
      <MoodShell {...moodProps} className="flex flex-col items-center justify-center p-4">
        <div className="mood-surface w-full max-w-md shadow-2xl p-8 text-center">
          ...
          <p className="text-[color:var(--mood-muted)] mb-4">
            {accessError || 'Invalid or expired access code'}
          </p>
          <a
            href={`/${username}/request`}
            className="mood-btn inline-flex px-4 py-2"
          >
            Enter access code
          </a>
        </div>
